...
from utils import check_gh_auth, load_config, fetch_project_items
...
def generate_weekly_report(config, output_dir, by_label=False, mode="target_date"):
    today = datetime.now().date()
    start_of_week = today - timedelta(days=today.weekday())
    end_of_week = start_of_week + timedelta(days=6)

    report_date_local = today.strftime("%Y-%m-%d")
    title_suffix = "Sprint Mode" if mode == "sprint" else "Target Date Mode"
    lines = [f"# 週次進捗サマリー ({report_date_local}) - {title_suffix}", ""]
    ...
    for proj in projects:
        ...
        if mode == "sprint":
            lines.append("| ✅ Done | 🟡 In Progress | 🔴 Todo / Other |")
            lines.append("| :------: | :------------: | :-------------: |")
            lines.append(f"| {len(done_items)} | {len(in_progress_items)} | {len(todo_items)} |")
            lines.append("")
            
            lines.append("### 🟡 対応中/未対応タスクリスト")
            incomplete_items = in_progress_items + todo_items
            if incomplete_items:
                lines.append("| Title | Status | Link |")
                lines.append("| ----- | ------ | ---- |")
                for task in incomplete_items:
                    ...
                    lines.append(f"| {title} | {status_str} | {link_str} |")
            else:
            ...
            if by_label:
                ...
                for item in due_this_week_items:
                    for l in item.get("labels", []) or ["Unlabeled"]:
    ...
    mode_suffix = "" if mode == "target_date" else f"_{mode}"
    report_path = output_dir_path / f"{report_date_local}{mode_suffix}.md"

    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    
    print(f"週次レポートを作成しました (Mode: {mode}): {report_path}")
...
def main():
    ...
    parser.add_argument("--mode", choices=["target_date", "sprint"], default="target_date", help="スプリントモードで出力します。")
    ...
    generate_weekly_report(config, output_dir, by_label=args.by_label, mode=args.mode)
